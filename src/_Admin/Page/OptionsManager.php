...
class OptionsManager
{
    ...
    public function options_assets()
    {
        ...
        wp_register_script('comment-notifier-js', $this->assets_url . 'js/manager-list.js', array('jquery'), '1.0.0',
            true);
    ...
    {
        $settings_link = '<a href="options-general.php?page=options_editor">' . __('Settings',
                'options_editor') . '</a>';
    ...
    public function wp_options_source($name)
    {
        ...
        // WP Core
        if (in_array($name, $this->wp_default_options) || in_array($name,
                $this->wp_vital_options) || preg_match('/_site_transient_timeout_poptags\w{3,}/',
                $name) || preg_match('/_site_transient_poptags\w{3,}/', $name)
        ) {
            $html .= "<a class='dashicons dashicons-wordpress source-dashicon' title='WordPress Core option'></a>";

            // Themes
        } else {
            if (preg_match('/theme_mods\w{3,}/', $name)) {
                $html .= "<a class='dashicons dashicons-admin-appearance source-dashicon' title='Theme option'></a>";

                // Jetpack
            } else {
                if (preg_match('/jetpack\w{3,}/', $name)) {
                    ...
                    if (preg_match('/woocommerce\w{3,}/', $name) || preg_match('/shop_\w{3,}_image_size/', $name)) {
                        ...
                        if (preg_match('/gform\w{3,}/', $name) || preg_match('/gravityform\w{3,}/',
                                $name) || preg_match('/rg_form\w{3,}/', $name)
                        ) {
     ...
     * @return string $html String with icon
     */
    public function get_options_delete_button($id)
    {

        if (!in_array($id, $this->wp_vital_options)) {
            return "<a href='javascript:void(0);' onclick=\"verify_option_deletion( '$id', '" . admin_url() . "options-general.php?page=options_editor&delete_option=$id&nonce=" . wp_create_nonce('wp_options_delete_' . $id) . "' );\" class='button-primary' />" . __('Delete Option',
                'options_editor') . "</a>";
        }
    }
    ...
    public function manager_delete_options()
    {
        ...
        if (isset($_GET['delete_option']) && $_GET['delete_option']) {
            ...
            if ($screen->id != 'settings_page_' . 'options_editor') {
                return;
            }
            ...
            if (wp_verify_nonce($_GET['nonce'], 'wp_options_delete_' . $_GET['delete_option'])) {
                ...
                $wpdb->delete($wpdb->options, array('option_id' => $_GET['delete_option']), array('%s'));
                if (1 == $wpdb->rows_affected) {
                    $optionIds = get_option('brisum_option_editor_option_ids');
                    $optionId = $_GET['delete_option'];
                    $optionKey = array_search($optionId, $optionIds);

                    if (false !== $optionKey) {
                        unset($optionIds[$optionKey]);
                        update_option('brisum_option_editor_option_ids', $optionIds);
                    }
                }
    ...
    function manager_ajax_update_option_callback()
    {
        ...
        $option = $wpdb->get_row($wpdb->prepare(
            ...
            WHERE option_id = '%d' LIMIT 1",
            $id
        ));

        echo htmlentities($option->option_value);
        die();
    }
    ...
    public function get_all_options_cacheless()
    {
        ...
        if (empty($optionIds)) {
            ...
            $alloptions_db = $wpdb->get_results(sprintf(
                "SELECT option_id, option_name, option_value FROM {$wpdb->options}
                WHERE option_id in (%s) ORDER BY option_name ASC",
                implode(',', array_map('intval', $optionIds))
            ));
        }
    ...
    public function settings_page()
    {
        ...
        $html .= "<a href='javascript:void(0);' class='button-primary add-option'>" . __('Add Options',
                'options_editor') . "</a>";

        $html .= "<form method='POST' action='" . admin_url() . "options-general.php?page=options_editor' class='add-option-form'>";
        ...
        $html .= "<th scope='col' class='manage-column column-source' style='width:7%;'>" . __('Source',
                'options_editor') . "</th>";
        $html .= "<th scope='col' class='sort manage-column column-name' data-sort='option-name'>" . __('Option Name',
                'options_editor') . "</th>";
        $html .= "<th scope='col' class='sort manage-column column-information' data-sort='option-value'>" . __('Option Data',
                'options_editor') . "</th>";
        $html .= "<th scope='col' class='manage-column column-date' style='width:14%;'>" . __('Actions',
                'options_editor') . "</th>";
        ...
        foreach ($all_options as $option) {
            $option = apply_filters('theme_options_option', $option);
            if (!$option) {
                continue;
            }
            ...
            $html .= "<td>" . $this->wp_options_source($option->option_name) . "</td>";
            $html .= "<td class='option-name'>$option->option_name</td>";
            $html .= "<td class='option-value'><div class='edit' id='$option->option_id'>" . htmlentities($option->option_value) . "</div></td>";
            $html .= "<td>" . $this->get_options_delete_button($option->option_id) . "</td>";
